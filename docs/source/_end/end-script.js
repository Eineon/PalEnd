document.addEventListener('DOMContentLoaded', () => {
  ...
  const tagData = {
    ...
    '起源': {
      type: `标签`,
      value: `一名角色最多只能习得1项<a href=##>起源</a>特技。`,
    },
    '原型': {
      type: `标签`,
    ...
    '绝技': {
      type: `标签`,
      value: `一名角色在每轮中最多只能宣言1次<a href=##>绝技</a>动作。`,
    },
    '先发': {
      type: `标签`,
    ...
    '解体': {
      type: `处境`,
      value: `角色失去了自己的凡世肉体，因此无法采取任何与之相关的行为。大部分角色的思维也会随之消散。`,
    },
    '狂乱': {
      type: `处境`,
      value: `狂乱。`,
    ...
    '持续伤害': {
      type: `处境`,
      value: `将受到持续伤害时的刻度视为第0刻，每当6的倍数份刻度被填充时，角色就会承受1次损伤，随后将持续伤害的强度减少1点。场景结束时，所有持续伤害都会自动解除。`,
    },
    '苦难': {
      type: `处境`,
    ...
    '迅捷': {
      type: `处境`,
      value: `在冲突命刻的每 <code>13-强度 (最低1)</code> 刻内仅1次，角色可以宣言1个额外的标准动作，并且这个动作产生的《|RE|》会减少4点。<br>
    ...
    '狂乱': {
      type: `处境`,
      value: `根据触发本处境的腐化，角色会获得相应的疯狂效应与EX动作。<br>
      作为一个自由动作，角色可以支付X点自信作为代价，发动以下效果之一。每个选项在一轮中最多只能发动1次。<br>
      ● 自身造成伤害时可以使用。选择1名受到伤害的角色作为对象，对象受到的DP获得 <code>X*(CR+被触发的腐化感染深度)</code> 点加值。<br>
      ● 自身受到伤害时可以使用。本次伤害对自身造成的DP受到 <code>X*(CR+被触发的腐化感染深度)</code> 点减值。本减值对于复合伤害减半。`,
    },
    '指示物': {
      type: `处境`,
  ...
  function replaceText() {
    ...
    while (node = walker.nextNode()) {
      ...
      const tagHtml = node.nodeValue.replace(regex, (_, textL, textK, textR) => {
        ...
        // 检查 tagInfo 是否存在
        if (!tagInfo) {
          return `<a href="##">{内容缺失：${textK}}</a>`; // 如果不存在，返回原始文本
        }
        // 如果存在，构建替换后的 HTML
