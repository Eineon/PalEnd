document.addEventListener('DOMContentLoaded', () => {
  ...
  const tagData = {
    ...
    '原型': {
      type: `标签`,
      value: `当角色习得一项<a href=##>原型</a>特技后，她可以在通过【自我完善】从职业领域中习得特技时，将相同的原型标签赋予这些特技。同一特技最多只能拥有1种原型标签。<br>
      对于已习得的特技，角色只能通过【自我完善】赋予原型标签，这些特技同样必须通过【自我完善】习得。`,
    },
    '唤醒': {
      type: `标签`,
    ...
    '重掷': {
      type: `效果`,
      value: `当角色受到能够重掷检定的效果时，需要重新投掷已经掷出的2D10，并使用新的结果计算骰值，即使新的结果比原来更差也是如此。同一次检定最多只能重掷1次。<br>
      当角色在具有「优势/劣势」的检定上受到能够重掷检定的效果时，由她自己决定重新投掷哪两枚D10。`,
    },
    '延展': {
      type: `效果`,
    ...
    '无心': {
      type: `种族特征`,
      value: `无心个体没有道德情结。个体的CR每比心灵效果的MR高1阶，就会在应对心灵效果的防御检定上获得2点加值。`,
    },
    ...
    '刻印': {
      type: `标签`,
      value: `如果同一项目同时受到多个<a href=##>刻印</a>类型相同的效果，新的效果必须成功反制现有效果才能生效。`,
    },
    '诅咒': {
      type: `标签`,
    ...
    '格挡': {
      type: `武器特性`,
      value: `安装这种武器的角色，抗击PRO将会获得1点精密加值。`,
    },
    '精制': {
      type: `武器特性`,
      value: `使用这种武器发动攻击时，AB与DP会获得1点内在加值。`,
    },
    '锐利': {
      type: `武器特性`,
      value: `使用这种武器发动攻击时，攻击检定的会心值可以获得1点内在加值。`,
    },
    '变通': {
      type: `武器特性`,
      value: `使用这种武器发动的攻击命中时，DP可以获得 <code>空置武器装备栏*2 (最高等于武器威力)</code> 点加值。`,
    },
    '演武': {
      type: `武器特性`,
    ...
    '捕获': {
      type: `武器特性`,
      value: `角色可以使用这种武器，对范围内的1名角色发动1次特殊的远程攻击。攻击对象必须进行1次应对本次攻击ACC的沉着防御。<br>
    ...
    '组合': {
      type: `武器特性`,
      value: `作为一个产生2点RE的【交互】，角色可以切换这种武器的形态。使用这种武器发动的攻击如果命中，则可以在攻击的清算阶段结束后，将这个【交互】作为附属动作执行1次。无法在空置的武器装备栏不足的情况下执行这个【交互】。`,
    },
    '轻型': {
      type: `武器特性`,
    ...
    '远攻': {
      type: `武器特性`,
      value: `使用这种武器对至近范围内的对象发动攻击时，攻击检定将会受到「劣势 1」。`,
    },
    '反冲': {
      type: `武器特性`,
    ...
    '弹容': {
      type: `武器特性`,
      value: `每次使用这种武器发动1次攻击，都会消耗1发弹药。弹药上限等于本特性的强度。如果弹药耗尽，则无法再使用这件武器发动攻击。作为一个具有<a href=##>操作</a>标签，并产生2点<a href=##>RE</a>的【交互】，角色可以重装1件武器的所有弹药。`,
    },
    ...
    '缓慢': {
      type: `处境`,
      value: `在冲突命刻的每 <code>强度</code> 刻内，角色只能宣言1个标准动作。每 <code>13-强度 (最低1)</code> 刻内，宣言的首个标准动作会额外产生4点《|RE|》。<br>
    ...
    '追击': {
      type: `处境`,
      value: `角色受到来自攻击动作的伤害时，将会激活追击，令本次伤害的DP获得等于追击强度的加值。随后追击将自动解除。<br>
      一名角色可以同时承受多个追击处境，但每次伤害最多只能激活1个追击。优先激活强度最高的追击。`,
    },
    '禁足': {
      type: `处境`,
  ...
  function replaceText() {
    ...
    while (node = walker.nextNode()) {
      ...
      const tagHtml = node.nodeValue.replace(regex, (_, textL, textK, textR) => {
        ...
        // 检查 tagInfo 是否存在
        if (!tagInfo) {
          return `<a href="##">{${textL || ''}${textK}${textR || ''}/.缺失描述}</a>`; // 如果不存在，返回原始文本
        }
        // 如果存在，构建替换后的 HTML
  ...
  tipEl.forEach(el => {
    ...
    const commentHtml = `<span class="CMT"><a href="##">${tip}</a>
      <span class="show">${tipText}</span></span>`;
