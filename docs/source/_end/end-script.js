document.addEventListener('DOMContentLoaded', () => {
  ...
  const tagData = {
    ...
    '唤醒': {
      type: `标签`,
      value: `角色最多只能习得1项<a href=##>唤醒</a>类型相同的特技。`,
    },
    '原型': {
      type: `标签`,
    ...
    '追击': {
      type: `处境`,
      value: `角色受到来自基本攻击的伤害时，将会激活追击，令本次攻击的DP获得等于追击强度的加值。随后追击将自动解除。<br>
      一名角色可以同时承受多个追击处境，但每次攻击最多只能激活1个追击。优先激活强度最高的追击。`,
    },
    '缓慢': {
      type: `处境`,
    ...
    '狂乱': {
      ...
      作为一个自由动作，角色可以支付X点自信作为代价，发动以下效果之一。Y等于 <code>CR+被触发的腐化感染深度</code>。每个选项在一轮中最多只能发动1次。<br>
      ● 自身造成伤害时触发。选择1名受到伤害的角色作为对象，对象受到的DP获得 <code>X*Y</code> 点加值。<br>
      ● 自身受到伤害时触发。本次伤害对自身造成的DP受到 <code>X*Y</code> 点减值。本减值对于复合伤害减半。`,
    },
    '标记': {
      type: `处境`,
        ...
        <table style="width: 100%">
          <tr><th><table style="width: 100%; font-size: 15px">
            <tr style="border-bottom: 0.5px solid white">
              ...
              <td style="width: 25%; text-align: center; border-left: 4px solid white">${tagInfo.type}</td>
            </tr>
          </table></th></tr>
